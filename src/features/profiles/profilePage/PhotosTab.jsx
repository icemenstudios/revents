...
export default function PhotosTab({ profile, isCurrentUser }) {
  ...
  async function handleDeletePhoto(photo, target){
     ...
     try {
        ...
        setUpdating({ isDeleting: false, target: null });
      }
  }
  return (
                ...
                <Card key={photo.id}>
                  ...
                  <Button.Group fluid widths={2}>
                    <Button
                      ...
                      content="Main"
                    />
                    <Button name={photo.id} onClick={(e) => handleDeletePhoto(photo, e.target.name)} loading={deleting.isDeleting && deleting.target === photo.id} 
                    disabled ={photo.url === profile.photoURL}
